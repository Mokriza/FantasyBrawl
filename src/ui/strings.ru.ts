...
export const UI = {
  ...
  terrainLegend: [
    ...
    { key: 'collapse', name: 'Обрушение', short: 'стена, 15 урона', movement: 'не пройти; кто остался на нём — 15 урона в начале хода', sight: 'обзор свободен' },
  ...
  passive: 'Пассивка',
  item: 'Артефакт',
  modifier: 'Модификатор арены',
  hold: 'Удержание точки',
  holdHint: 'Раунды, в начале которых ваш герой (слева) или герой противника (справа) стоял на центральном гексе. Кто наберёт нужное число, побеждает',
  ...
  menu: {
    ...
    difficulty: 'Противник',
    difficulties: {
      novice: { name: 'Новичок', hint: 'Часто ошибается, не бережёт героев, не пользуется способностями тира IV' },
      normal: { name: 'Обычный', hint: 'Разумно выбирает ход, но смотрит только на его итог' },
      veteran: { name: 'Ветеран', hint: 'Просчитывает ответ соперника на свой ход' },
      nightmare: { name: 'Кошмар', hint: 'Просчитывает два хода вперёд и не ошибается' },
    },
  },
  ...
  draft: {
    ...
    budgetItem: 'артефакт',
    budgetReserveHint: 'Пассивка и тир IV — очки, отложенные под то, что герой выберет между матчами',
    tier: 'тир',
  ...
  upgrade: {
    ...
    hint: 'Каждый герой получает уровень, выбирает один перк из трёх, а после 1-го и 2-го матча — ещё пассивку и способность тира IV. Одного героя можно заменить. Выбор можно поменять до перехода к расстановке',
    modifierNext: (match: number) => `Матч ${match} пройдёт с модификатором арены:`,
    catchUp: 'Вы отстаёте на две победы: на выбор на один перк и один артефакт больше',
    enemyCatchUp: 'Противник отстаёт на две победы: у него на выбор на один вариант больше',
    swapTitle: 'Замена героя — по желанию, одного за фазу',
    swapShow: 'Показать кандидатов',
    swapHide: 'Скрыть кандидатов',
    swapHint: 'Кандидат приходит на уровне команды: пассивка, способность тира IV и перки уже выбраны, у него обычный артефакт. Ушедший герой покидает забег',
    swapInstead: 'Вместо:',
    swapChosen: 'Придёт вместо',
    swapCancel: 'Отменить замену',
    leaving: 'Уходит из команды, вместо него —',
    newcomer: 'Новичок',
    newcomerHint: 'Пришёл готовым: выбирать за него нечего',
    enemySwap: 'Замена',
    ...
    perkTitle: 'Перк',
    rewardTitle: 'Награда за матч: выберите артефакт и героя',
    rewardTitleShort: 'Награда',
    rewardWho: 'Кому отдать?',
    rewardFor: '→',
    replaces: 'заменит',
    itemTiers: { common: 'Обычный', rare: 'Редкий', legendary: 'Легендарный' },
...
const VICTORY: Record<VictoryReason, { won: string; lost: string }> = {
  ...
  hold: { won: 'Вы удержали Точку силы', lost: 'Противник удержал Точку силы' },
};
