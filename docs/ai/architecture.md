...
core/
  ...
  arena/            генерация арены, препятствия, симметрия
  battle/
...
- **Онлайн:** сервер на Node запускает тот же `core`. Клиенты шлют `Action`, сервер проверяет через `legalActions` и рассылает `Action` + `events`. Синхронизация = список действий.
